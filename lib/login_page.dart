...
const String baseUrl = "http://168.144.47.209:2321";
...
class _LoginPageState extends State<LoginPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _initAnim();
    getAndroidId().then((id) => androidId = id);
  }
  ...
  Future<void> login() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && data['status'] == "success") {
        ...
        _showPopup(title: "❌ Gagal", message: data['message'] ?? "Akses ditolak!");
      }
    } catch (e) {
      _showPopup(title: "⚠️ Error", message: "Gak nyambung ke server, stupid!");
    } finally {
      setState(() => isLoading = false);
    }
  }

  void _showPopup({required String title, required String message, Color color = Colors.red}) {
    showDialog(
      context: context,
      ...
      body: Center(
        child: Form(
          ...
          child: Column(
            ...
            children: [
              const Text("THE SHADOW OF RVPA", style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
              const SizedBox(height: 30),
              Padding(
                padding: const EdgeInsets.all(20.0),
                child: TextFormField(controller: userController, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: "User", labelStyle: TextStyle(color: Colors.white))),
              ),
              Padding(
                padding: const EdgeInsets.all(20.0),
                child: TextFormField(controller: passController, obscureText: true, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: "Pass", labelStyle: TextStyle(color: Colors.white))),
              ),
              const SizedBox(height: 20),
              isLoading ? const CircularProgressIndicator() : ElevatedButton(onPressed: login, child: const Text("LOGIN MAESTRO")),
            ],
          ),
        ),
      ),
